<script lang="ts">
  ...
  let { mode = $bindable(), onSwitch }: { mode: string; onSwitch: (mode: string) => void } = $props();
...
<section class="bg-white dark:bg-zinc-900 border border-slate-300 dark:border-zinc-700 transition-colors {r ? 'rounded-xl' : ''}">
  ...
  <div class="p-4">
    <div class="flex text-sm font-bold w-full text-center">
      {#each modes as m, i}
        <button
          class="flex-1 py-2.5 transition-[transform,colors] duration-200 outline-none border -ml-px first:ml-0 active:translate-y-px active:scale-[0.99]
          {r ? (i === 0 ? 'rounded-l-lg' : i === modes.length - 1 ? 'rounded-r-lg' : '') : ''}
          {mode === m
