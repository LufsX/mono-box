<script lang="ts">
  ...
  function triggerClass(): string {
    const base = "w-full flex items-center justify-between px-3 py-1.5 border outline-none transition-colors disabled:opacity-50 disabled:cursor-not-allowed rounded-lg";
  ...
  function optionClass(isSelected: boolean): string {
    ...
    return isSelected ? `${base} bg-slate-100 dark:bg-zinc-800 text-slate-900 dark:text-slate-100 font-bold` : `${base} text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-zinc-800`;
  }
</script>

<div class="relative" bind:this={containerRef}>
  <button {id} type="button" {disabled} onclick={() => (open = !open)} aria-expanded={open} class={triggerClass()}>
  ...
  {#if open}
    <div in:fly={{ y: -5, duration: 150, easing: cubicOut }} out:fade={{ duration: 100 }} class={menuClass()}>
      <div class="max-h-60 overflow-y-auto flex flex-col">
        {#each options as option (option.value)}
          <button type="button" class={optionClass(value === option.value)} onclick={() => selectOption(option.value)}>
