<script lang="ts">
  ...
  function parseClearMessage(stdout: string): string {
    for (const line of stdout.split(/\r?\n/)) {
      const parts = line.trim().split("\t");
      if (parts[0] !== "cleared") continue;

      const bytes = Number.parseInt(parts[1] || "0", 10);
      const count = Number.parseInt(parts[2] || "0", 10);
      const failed = Number.parseInt(parts[3] || "0", 10);
      if (failed > 0) {
        return `已清理 ${formatBytes(bytes)}，${failed} 个文件失败`;
      }
      return `已清理 ${formatBytes(bytes)} / ${count} 个文件`;
    }
  ...
  async function clearLogFiles() {
    ...
    try {
      ...
      clearMessage = parseClearMessage(result.stdout);
...
<main class="max-w-3xl mx-auto px-4 py-6 min-h-full flex flex-col gap-4">
  ...
  <section class="border border-zinc-800 bg-zinc-950 overflow-hidden rounded-xl">
    <div class="flex flex-col gap-3 border-b border-zinc-800 bg-zinc-900 px-3 py-3">
      <div class="grid grid-cols-[minmax(0,1fr)_auto] gap-3 items-center">
        <div class="w-36 text-xs font-bold">
          <Select id="log-level" options={LEVEL_OPTIONS} bind:value={selectedLevel} onchange={(value) => (selectedLevel = value as LogLevelFilter)} />
        </div>
    ...
    <div class="h-[52vh] min-h-96 overflow-y-auto bg-black p-3 font-mono text-xs leading-relaxed custom-scrollbar">
      {#if logRows.length === 0}
        <div class="flex h-full items-center justify-center text-zinc-700">Waiting for logs...</div>
      {:else if filteredLogRows.length === 0}
        <div class="flex h-full items-center justify-center text-zinc-700">无匹配日志</div>
      {:else}
        <div class="space-y-2">
          {#each filteredLogRows as row (row.id)}
            <div class="border-b border-zinc-900 pb-2 text-zinc-300" in:fade={{ duration: 120 }}>
              <div class="flex items-center justify-between gap-3">
                <span class={typeClass(row.type)}>{row.type}</span>
                <time class="shrink-0 select-none text-zinc-600">[{row.time}]</time>
              </div>
              <div class="mt-2 break-words whitespace-pre-wrap text-zinc-300">{row.payload}</div>
            </div>
          {/each}
        </div>
      {/if}
  ...
  <section class="border border-slate-300 dark:border-zinc-700 bg-white dark:bg-zinc-900/90 rounded-xl overflow-hidden">
    <div class="flex items-center justify-between gap-3 border-b border-slate-300 dark:border-zinc-700 bg-slate-50 dark:bg-zinc-950 px-4 py-3">
      ...
      <div class="flex items-center gap-2 shrink-0">
        ...
        <button
          type="button"
          class="inline-flex items-center gap-2 h-8 px-3 border border-rose-200 bg-rose-50 hover:bg-rose-100 text-rose-600 dark:border-rose-900/40 dark:bg-rose-950/30 dark:hover:bg-rose-900/50 dark:text-rose-400 text-xs font-bold transition-colors disabled:opacity-60 rounded-lg"
          onclick={clearLogFiles}
          disabled={clearingFiles || logReport.totalBytes === 0}
          title="清理日志"
        >
          <Trash2 size={14} />
          <span>{clearingFiles ? "清理中" : "清理日志"}</span>
        </button>
      </div>
    </div>
    ...
    <div class="p-4 space-y-3">
      {#if fileError}
        <div class="border border-red-300 dark:border-red-900/50 bg-red-50 dark:bg-red-950/30 px-3 py-2 text-xs text-red-700 dark:text-red-300" in:fly={{ y: -6, duration: 180, easing: cubicOut }}>
          {fileError}
        </div>
      {/if}
      ...
      {#if clearMessage}
        <div
          class="border border-emerald-300 dark:border-emerald-900/40 bg-emerald-50 dark:bg-emerald-950/20 px-3 py-2 text-xs text-emerald-700 dark:text-emerald-300"
      ...
      {#if logFileLoading && sortedLogFiles.length === 0}
        <div class="py-8 text-center text-sm text-slate-500 dark:text-zinc-400">正在加载日志文件...</div>
      {:else if sortedLogFiles.length === 0}
        <div class="py-8 text-center text-sm text-slate-500 dark:text-zinc-400">暂无日志文件</div>
      {:else}
        <div class="space-y-2">
          {#each sortedLogFiles as file (file.path)}
            <article class="grid grid-cols-[minmax(0,1fr)_auto] gap-3 items-center border border-slate-200 dark:border-zinc-800 bg-slate-50/70 dark:bg-zinc-950/40 px-3 py-2 rounded-lg">
              <div class="min-w-0">
