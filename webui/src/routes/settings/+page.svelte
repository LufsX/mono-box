<script lang="ts">
  ...
  async function loadBoxConfig() {
    try {
      ...
      boxConfigPort = portMatch ? parseInt(portMatch[1], 10) : 9090;

      // 解析 clash_api_secret
      const secretMatch = content.match(/clash_api_secret=(.+)/);
      boxConfigSecret = secretMatch ? secretMatch[1].trim() : "";
    } catch (e) {
      boxConfigError = "无法读取 box.config 文件";
  ...
  async function saveBoxConfig() {
    try {
      ...
      await clashApi.updateBoxConfigValue("clash_api_port", boxConfigPort.toString());
      await clashApi.updateBoxConfigValue("clash_api_secret", boxConfigSecret);
      
      boxConfigSaved = true;
      setTimeout(() => (boxConfigSaved = false), 2000);
    } catch (e) {
      boxConfigError = "保存失败，请检查权限";
...
<div class="max-w-3xl mx-auto px-4 py-6 min-h-screen flex flex-col gap-6">
    ...
    <div class="p-4 space-y-4">
      ...
      <div class="flex gap-3 pt-2">
        <button
          ...
          disabled={boxConfigLoading}
          class="flex-1 border border-slate-800 dark:border-slate-400 bg-slate-800 dark:bg-slate-200 text-white dark:text-zinc-900 px-4 py-2.5 text-sm font-bold hover:bg-slate-700 dark:hover:bg-slate-300 transition-colors outline-none disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {boxConfigLoading ? "保存中..." : "保存配置"}
        </button>
        {#if boxConfigSaved}
          <div class="flex items-center text-sm font-bold text-green-600 dark:text-green-400">✓ 已保存</div>
        {/if}
      </div>

      <div class="px-3 py-2 bg-blue-50 dark:bg-blue-950 border border-blue-300 dark:border-blue-800 text-blue-700 dark:text-blue-300 text-xs">
        注意：修改配置后需要重启服务才能生效
      </div>
    </div>
  </section>
